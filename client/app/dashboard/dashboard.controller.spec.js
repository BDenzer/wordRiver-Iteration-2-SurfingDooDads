...
describe('Controller: DashboardCtrl', function () {
  ...
  it('addStudent test', function () {
    scope.studentList =[{_id:"Word",firstName:"TestPerson"}]
    scope.currentGroup = {students: []};
    scope.addStudent();
    expect(scope.currentGroup.students.length).toBe(0);
    scope.studentDropDown = "Word";
    scope.addStudent();
    expect(scope.currentGroup.students.length).toBe(1);
    scope.addStudent();
    expect(scope.currentGroup.students.length).toBe(1);
    expect(scope.currentGroup.students[0].firstName).toBe("TestPerson");

  });

  it('deleteStudent test', function () {
    scope.currentGroup = {students: []};
    scope.currentGroup.students = [{firstName: "Word", lastName: "Type"}, {firstName: "New", type: "Type"}, {firstName: "Pack", type: "Type"}, {firstName: "WordPack", type: "Type"}];
    scope.deleteStudent(scope.currentGroup, 2);
    expect(scope.currentGroup.students[0].firstName).toBe("Word");
    expect(scope.currentGroup.students[1].firstName).toBe("New");
    expect(scope.currentGroup.students[2].firstName).toBe("WordPack");
    scope.deleteStudent(scope.currentGroup, 0);
    expect(scope.currentGroup.students[0].firstName).toBe("New");
    expect(scope.currentGroup.students[1].firstName).toBe("WordPack");
    scope.deleteStudent(scope.currentGroup, 1);
    expect(scope.currentGroup.students[0].firstName).toBe("New");
    scope.deleteStudent(scope.currentGroup,0);
    //expect(scope.currentGroup..length).toBe(0);
